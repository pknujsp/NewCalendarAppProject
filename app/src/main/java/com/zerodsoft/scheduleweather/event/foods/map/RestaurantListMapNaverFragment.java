package com.zerodsoft.scheduleweather.event.foods.map;
...
public class RestaurantListMapNaverFragment extends NaverMapFragment implements OnClickedPlacesListListener, OnBackPressedCallbackController, PlaceCategory
{
    public static final String TAG = "RestaurantListMapNaverFragment";
    ...
    public RestaurantListMapNaverFragment()
    {
        super();
    }
    ...
    public void onPageSelectedPlaceBottomSheetViewPager(int position)
    {
        if (adapter.getItemCount() - 1 == position)
        {
            ...
            {
                if (placesOfSelectedCategoriesFragment.isVisible() && foodMenuChipGroup.getCheckedChipIds().size() >= 1)
                {
    ...
    {
        if (foodMenuChipGroup.getChildCount() >= 3)
        {
            foodMenuChipGroup.removeViews(2, foodMenuChipGroup.getChildCount() - 2);
        }
        chipMap.clear();
        ...
        for (PlaceCategoryDTO placeCategory : placeCategoryList)
        {
            ...
            final RestaurantListMapNaverFragment.ChipViewHolder chipViewHolder = new RestaurantListMapNaverFragment.ChipViewHolder(placeCategory, index++);
    ...
    private void addListChip()
    {
        ...
        foodMenuChipGroup.addView(listChip, 0);
    }
    ...
    private void addSettingsChip()
    {
        ...
        foodMenuChipGroup.addView(settingsChip, 1);
    }
    ...
    private final ActivityResultLauncher<Intent> placeCategoryActivityResultLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(),
            new ActivityResultCallback<ActivityResult>()
            {
                ...
                public void onActivityResult(ActivityResult result)
                {
                    ...
                    {
                        if (foodMenuChipGroup.getCheckedChipIds().size() > 0)
                        {
        ...
        public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked)
        {
           ...
           poiitem이 선택된 경우 해제하고, poiitem을 새로 생성한 뒤 poiitem전체가 보이도록 설정
            ...
            if (isChecked)
            {
                ...
                PlaceCategoryDTO placeCategory = ((RestaurantListMapNaverFragment.ChipViewHolder) compoundButton.getTag()).placeCategory;
